...
def mark_attendance():
        ...
        for (x, y, w, h) in faces:
            ...
            if conf < 80 and label in label_map:
                ...
                filename_csv = os.path.join(
                    ATTENDANCE_FOLDER, f"attendance_{datetime.now().strftime('%Y%m%d')}.csv"
                )
    ...
    if not recognized:
        ...
        messagebox.showinfo(
            "No Match",
            "No registered face matched. Please try again or ask teacher to register the student photo."
        )
...
btn_start = Button(frm, text="Start Attendance Window", width=24, bg="#4a6cf7", fg="white",
                    command=start_attendance_window)
btn_start.grid(row=0, column=2, padx=(8, 0), pady=(0, 6))

Label(frm, text="Pictures folder (for registered photos):", bg="#f6f6f6").grid(
    row=1, column=0, columnspan=2, sticky=W)
Label(frm, text=PICTURES_FOLDER, bg="#f6f6f6", fg="black").grid(row=1, column=2, sticky=W)

btn_train = Button(frm, text="Train from pictures", width=24, bg="#28a745", fg="white",
                    command=train_from_pictures)
btn_train.grid(row=2, column=0, pady=(10, 6))

btn_mark = Button(frm, text="Mark Attendance (student)", width=24, bg="#f0ad4e", fg="black",
